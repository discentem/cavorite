...
func (s *S3Store) Retrieve(ctx context.Context, mmap metadata.CfileMetadataMap, objects ...string) error {
	...
	for _, o := range objects {
		...
		logger.Infof("mmap: %s", mmap)
		mmapKey := fmt.Sprintf("%s.%s", o, s.Options.MetadataFileExtension)
		m, ok := mmap[mmapKey]
		if !ok {
			result = multierr.Append(result, fmt.Errorf("%q not found in mmap", mmapKey))
			continue
		}
		...
		matches, err := metadata.HashFromCfileMatches(s.fsys, o, m.Checksum)
		if err != nil {
			result = multierr.Append(result, err)
			continue
		}
