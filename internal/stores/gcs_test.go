...
func fakeGCSStore(t *testing.T) Store {
	// create fake gcs
	server, err := fakestorage.NewServerWithOptions(fakestorage.Options{
		...
		Port:           3225,
		NoListener:     false,
		PublicHost:     "http://localhost:3225",
	})
	server.CreateBucket("test")
	assert.NoError(t, err)

	client, err := storage.NewClient(
		context.Background(),
		option.WithEndpoint(fmt.Sprintf("%s/storage/v1", server.URL())),
