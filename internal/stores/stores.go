...
func WriteMetadataToFsys(s Store, o string) (cleanup func() error, err error) {
	opts := s.GetOptions()
	...
	if err := fsys.MkdirAll(filepath.Dir(filepath.Dir(o)), os.ModePerm); err != nil {
		return nil, err
	}
	// Write metadata to disk
	metadataPath := fmt.Sprintf("%s.%s", o, opts.MetadataFileExtension)
